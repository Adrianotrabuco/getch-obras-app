...
const appVersion = '39-servico-certo';
...
function notifyNewTasks(newTasks) {
  ...
  if ('Notification' in window && Notification.permission === 'granted') {
    new Notification('Servico Certo', { body: message });
  } else {
    alert(message);
  }
}
...
async function shareEvidence(id) {
  ...
  const message = [
    ...
    ``,
    `Servico Certo - evidencias da tarefa`,
    `Tarefa: ${task.title}`,
...
function alertInventoryMove(id) {
  ...
  const message = [
    'Servico Certo - aviso de mudanca de ferramenta',
    `Item: ${item.name}`,
